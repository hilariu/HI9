const API_BASE = "http://localhost:3001"; // ajuste se precisar
...
async function apiPost(path, body) {
    ...
    if (!res.ok) throw new Error(data.error || "Erro POST " + path);
    return data;
}
...
async function apiPut(path, body) {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.error || "Erro PUT " + path);
    return data;
}
...
async function apiDelete(path) {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.error || "Erro DELETE " + path);
    return data;
}
